 ...
 *
 * @author David Kimball & Kenneth Gayol
 */
...
void EduBase_LCD_Ports_Init(void)
{
	...
	// Configure the PA5, PA4, and PA3, and PA2 pins to function as
	...
	// Enable the lcock to Port C by setting the
	...
	// Initialize the output of the PC6 to zero
	...
	// Enabe the clock to Port E by setting the
	// R4 bit (Bit 4) in the RCGCGPIOO register
...
void EduBase_LCD_Pulse_Enable(void)
{
	...
	// in the DATA register high anc learing it after 1 us.
	// The minimum time for the enable pulse width must be at least greater than 450 ns
	// during a register / write operation (page 49 of HD44780 datasheet)
	GPIOC->DATA |= 0x40;
	SysTick_Delay1us(1);
	GPIOC->DATA &= ~0x40;
}
...
void EduBase_LCD_Send_Command(uint8_t command)
{
	...
	// Transmit the lower nible of the command byte
...
void EduBase_LCD_Init(void)
{
	...
	// Transmit function set initialize commands as part of the LCD initialization sequence
	...
	// Transmit a Function Set sommand to the LCD to configure it to use 4-bit mode
	...
	// Transmit a Retunr Home command to set the DDRAM address to 0 and to return
